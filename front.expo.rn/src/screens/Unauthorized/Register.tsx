...
export default function Register({doLogin}: {doLogin: (afterJob: () => void, credential: UserCredential) => void}){
    ...
    return <KeyboardAvoidingView 
        ...
        >
            {isPassedToSecondStage ? <SecondStage bottomInset={bottomInset} stageSetter={setIsPassedToSecondStage} /> : 
...
function RegisterMain({bottomInset, stageSetter} : {bottomInset: number, stageSetter: (value: boolean) => void}) {
    ...
    const handleUidValidation = (value: string, onBlur: () => void) => {
        setChanged(_ => !_);
        ...
        style={{ paddingBottom: bottomInset * 2, paddingTop: bottomInset }}>
            <View className="w-full">
                ...
                <Controller
                    ...
                    name="uid"
                />
                {errors.uid && <Text className="text-red-500">{errors.uid.message as string}</Text>}
            </View>
            <View className="w-full">
